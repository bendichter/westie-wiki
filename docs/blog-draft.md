...
West Coast Swing has a knowledge problem. It's a living, improvised dance. Patterns mutate, names collide, and every scene calls the same move something different, so its collective knowledge lives in workshop notes, YouTube comments, and the heads of teachers. When you're learning, you hear a name like "sugar tuck" and have no way to look it up, compare what different pros do with it, or figure out what to learn next.

Wikipedia solved this shape of problem for facts. I wanted the same thing for moves: pages anyone can edit, every claim backed by video evidence, and revision history so nothing is ever lost. One caveat worth stating up front, since a dance is not an encyclopedia: the wiki describes how people actually dance and what they call things. It's a learning aid. If your teacher disagrees with a page, listen to your teacher, then edit the page.

## What it does

- **Moves.** Each pattern gets a page: a description with counts and technique notes, alternative names (collisions are a feature, not a bug), difficulty, tags, and prerequisite/variation relationships that turn the catalog into a browsable skill graph.
...
- **Dances.** My favorite part. Instead of hunting clips for one move, you register a full competition video (dancers, event, division, placement, setlist) and then annotate it while it plays: hit "now" as each pattern starts, name the move, repeat. Every annotation becomes a labeled clip on that move's page automatically. There are 64 real dances indexed, from US Open finals to Budafest invitationals, waiting to be mapped.
